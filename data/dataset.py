...
LE_ROBOT_TEMP_STATS_FILENAME = "meta/temp_stats.json"
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        modifiy_tasks_num: dict[str, int] | None = None,
        ...
        Args:
            ...
            modifiy_tasks_num (dict[str, int]): A dictionary to modify the number of tasks.
            balance_tasks (bool): Whether to balance the tasks.If True, the dataset will oversample tasks to have equal number of episodes.
        """
        ...
        self._tasks_map_to_id= self._get_tasks_map_to_id(modifiy_tasks_num)
        if balance_tasks:
            self._tasks_map_to_id = self.balance_task_data(self._tasks_map_to_id)
    ...
    def _get_tasks_map_to_id(self, modifiy_tasks_num: str) -> dict[str, list[int]]:
        """Get the tasks map to id for the dataset."""
        episode_path = self.dataset_path / LE_ROBOT_EPISODE_FILENAME
        with open(episode_path, "r") as f:
            episode_metadata = [json.loads(line) for line in f]
        tasks_map_to_id = {}
        for episode in episode_metadata:
            if episode["tasks"][0] not in tasks_map_to_id:
                tasks_map_to_id[episode["tasks"][0]] = []
            tasks_map_to_id[episode["tasks"][0]].append(episode["episode_index"])
        if modifiy_tasks_num:
            for task in modifiy_tasks_num:
                if task in tasks_map_to_id:
                    tasks_map_to_id[task] = tasks_map_to_id[task][:modifiy_tasks_num[task]]
                else:
                    raise ValueError(f"Task {task} not found in dataset {self.dataset_name}")
        return tasks_map_to_id
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        temp_stats_path = self.dataset_path / LE_ROBOT_TEMP_STATS_FILENAME
        try:
        ...
        modified_episode_ids = []
        modified_parquet_files = []
        for _, episode_ids in self._tasks_map_to_id.items():
            modified_episode_ids.extend(episode_ids)
        for p in self.dataset_path.glob(LE_ROBOT_DATA_FILENAME):
            num_str = p.stem.split("_")[-1]
            ep_id = int(num_str)
            if ep_id in modified_episode_ids:
                modified_parquet_files.append(p)
        le_statistics_modified = calculate_dataset_statistics(modified_parquet_files)
        with open(temp_stats_path, "w") as f:
            json.dump(le_statistics_modified, f, indent=4)
        dataset_statistics = {}
        for our_modality in ["state", "action"]:
            ...
            for subkey in simplified_modality_meta[our_modality]:
                ...
                for stat_name in le_statistics_modified[le_modality]:
                    indices = np.arange(
                        state_action_meta.start,
                        state_action_meta.end,
                    )
                    stat = np.array(le_statistics_modified[le_modality][stat_name])
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        trajectory_lengths = []
        for episode in episode_metadata:
            if episode["episode_index"] not in self._tasks_map_to_id[episode['tasks'][0]]:
                continue
            count = self._tasks_map_to_id[episode['tasks'][0]].count(episode["episode_index"])
    ...
    def balance_task_data(self, data_dict: dict) -> dict:
        """Balance the task data by oversampling tasks to have equal number of episodes.

        Args:
            data_dict (dict): The data dictionary to balance.

        Returns:
            dict: The balanced data dictionary.
        """
        if not data_dict:
            return {}
        max_len = max(len(v) for v in data_dict.values())
        balanced_dict = {}
        for task_name, episode_list in data_dict.items():
            curr_len = len(episode_list)
            if curr_len == 0:
                balanced_dict[task_name] = []
                continue
            
            full_repeats = max_len // curr_len
            remainder = max_len % curr_len
            
            new_list = episode_list * full_repeats + episode_list[:remainder]
            
            balanced_dict[task_name] = new_list
    ...
    def get_trajectory_index(self, trajectory_id: int) -> int:
        ...
        if len(trajectory_indices) != 1 and self.balance_tasks is False:
            raise ValueError(
