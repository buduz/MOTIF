...
from data.embodiment_tags import EMBODIMENT_TAG_MAPPING, EmbodimentTag
from data.schema import DatasetMetadata
from data.transform.base import InvertibleModalityTransform
...
class DefaultDataCollator(DataCollatorMixin):
    def __init__(self, eagle_path: str):
        super().__init__()
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        images = self._prepare_video(data)  # v t c h w
        ...
        # 4) Add rel_state and progress
        if "rel_state" in data:
            transformed_data["rel_state"] = data['rel_state']
        if "progress" in data:
            transformed_data["progress"] = data['progress']
    ...
    def apply_batch(self, data: dict, batch_size: int) -> dict:
        ...
        data_split = [
            tree.map_structure(
                lambda x, i=i: x if isinstance(x, str) else x[i],
                data
            )
        ...
        return MOTIFCollator()(data_split_processed)
