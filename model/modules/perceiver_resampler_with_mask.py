...
class PerceiverAttentionLayer(nn.Module):
    ...
    def forward(self, features, latents, key_attention_mask=None):
        """Latents attend to both input features and other latents
           (keys/values are [features; latents]).
           key_attention_mask only applies to the feature part.
        ...
        if key_attention_mask is not None:
            ...
            # create full mask including latents
...
class PerceiverResampler(nn.Module):
    ...
    def forward(self, x_f: torch.Tensor, key_padding_mask: torch.BoolTensor = None):
        ...
        Args:
            ...
            key_padding_mask: Mask for the input embeddings of shape (batch_size, n_features), where True/1 indicates valid tokens
        ...
        pos_ids = key_padding_mask.long().cumsum(dim=1) - 1 
        pos_ids = pos_ids.clamp(min=0)
        pos_emb = self.time_pos_emb[pos_ids]  
